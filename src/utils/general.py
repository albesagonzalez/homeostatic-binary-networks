...
def get_latent_accuracy(X, selectivity, labels, active_threshold=0.0):
    """
    Compute per-latent accuracy using neuron→latent assignments from selectivity.

    Args:
        X: (T, N) activity tensor (binary or real). A neuron is considered active
           at t if X[t, n] > active_threshold.
        selectivity: (N, L) tensor; for each neuron its preferred latent is
           argmax over L.
        labels: (T, L) tensor; per-time one-hot/indicator of the true latent.
               Non-zero is treated as positive for that latent.
        active_threshold: float threshold to deem a neuron active.

    Returns:
        (L,) tensor with mean accuracy per latent (fraction of times that
        latent was the label and the predicted latent matched it). If a latent
        never appears in labels, its accuracy is 0.
    """
    # Ensure tensors
    if not torch.is_tensor(X):
        X = torch.tensor(X)
    if not torch.is_tensor(selectivity):
        selectivity = torch.tensor(selectivity)
    if not torch.is_tensor(labels):
        labels = torch.tensor(labels)

    T, N = X.shape
    N_sel, L = selectivity.shape
    assert N_sel == N, f"selectivity has N={N_sel}, but X has N={N}"
    assert labels.shape[0] == T and labels.shape[1] == L, (
        f"labels shape {tuple(labels.shape)} incompatible with T={T}, L={L}"
    )

    # Assign each neuron to the latent with max selectivity
    neuron_latent = torch.argmax(selectivity, dim=1)  # (N,)

    # Active neurons per time
    active = (X > active_threshold).to(torch.float32)  # (T, N)

    # Build one-hot for neuron assignments: (N, L)
    one_hot = torch.nn.functional.one_hot(neuron_latent, num_classes=L).to(torch.float32)

    # Count active neurons supporting each latent at each time: (T, L)
    counts = active @ one_hot

    # Predicted latent per time
    pred = torch.argmax(counts, dim=1)  # (T,)

    # Compute per-latent accuracy
    acc = torch.zeros(L, dtype=torch.float32)
    for l in range(L):
        mask = labels[:, l] != 0  # treat non-zero as positive
        if mask.any():
            acc[l] = (pred[mask] == l).to(torch.float32).mean()
        else:
            acc[l] = 0.0
